...
public class BackControl : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        this.HP = Box.MAX_HP;
        vel = Box.MOVE_SPEED;
        OC = GameObject.Find("Bikes").GetComponent<OrderChange>();
    }
    ...
    void Update()
    {
        ...
        */
        if (OC.changeFlag == 0)this.transform.LookAt(Flt.transform);
        Vector3 vecFront = (Flt.transform.position - transform.position).normalized;
