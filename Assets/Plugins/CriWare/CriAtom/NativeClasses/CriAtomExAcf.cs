...
public class CriAtomExAcf
{
    ...
    public struct AcfDspBusInfo
    {
        ...
        public AcfDspBusInfo(byte[] data, int startIndex)
        {
            if (IntPtr.Size == 4)
            {
                ...
                for (int i = 0; i < 8; ++i)
                {
                    fxIndexes[i] = BitConverter.ToUInt16(data, startIndex + 20 + (2 * i));
                }
                this.busLinkIndexes = new ushort[64];
                for (int i = 0; i < 64; ++i)
                {
                    busLinkIndexes[i] = BitConverter.ToUInt16(data, startIndex + 36 + (2 * i));
                }
                this.busNo = BitConverter.ToUInt16(data, startIndex + 164);
                this.numFxes = data[startIndex + 165];
                this.numBusLinks = data[startIndex + 166];
            }
            else
            {
                this.name = Marshal.PtrToStringAnsi(new IntPtr(BitConverter.ToInt64(data, startIndex + 0)));
    ...
    */
    ...
    {
        normal = 0,             /*JP< ノーマルタイプ                */
        autoModulation,         /*JP< オートモジュレーションタイプ  */
    }
    ...
    */
    ...
    public static bool GetGlobalAisacGraphInfo(GlobalAisacInfo aisacInfo, ushort graphIndex, out AisacGraphInfo graphInfo)
    {
        ...
        {
            bool result = criAtomExAcf_GetGlobalAisacGraphInfo(ptr, graphIndex, mem.ptr);
    ...
    public static bool GetSelectorLabelInfo(SelectorInfo selectorInfo, ushort labelIndex, out SelectorLabelInfo info)
    {
        ...
        {
            bool result = criAtomExAcf_GetSelectorLabelInfo(ptr, labelIndex, mem.ptr);
